...
pub fn render_preview(ctx: &EditorCtx) {
    ...
    let preview_theme = ctx.state.borrow().preview_theme.clone();
    // Set the WebView background to match the theme immediately,
    // preventing a flash of the wrong color while HTML loads.
    let bg = match preview_theme.as_str() {
        "sepia" => gtk::gdk::RGBA::new(0.957, 0.925, 0.847, 1.0),     // #f4ecd8
        "solarized" => gtk::gdk::RGBA::new(0.992, 0.965, 0.89, 1.0),  // #fdf6e3
        _ if dark => gtk::gdk::RGBA::new(0.118, 0.118, 0.118, 1.0),   // #1e1e1e
        _ => gtk::gdk::RGBA::new(0.98, 0.98, 0.98, 1.0),              // #fafafa
    };
    ctx.preview_webview.set_background_color(&bg);
    let mut html = build_preview_html(&markdown, dark, &preview_theme);
...
/// Convert markdown to HTML body (no wrapper/styles). Used for "Copy as HTML".
...
pub fn build_preview_html(markdown: &str, dark: bool, preview_theme: &str) -> String {
    ...
    let (bg, fg, code_bg, border, link_color, heading_color) = match preview_theme {
        "sepia" => ("#f4ecd8", "#5b4636", "#ebe0cc", "#c9b99a", "#8b5e3c", "#3d2b1f"),
        "solarized" => ("#fdf6e3", "#657b83", "#eee8d5", "#d3cbb7", "#268bd2", "#586e75"),
        _ if dark => ("#1e1e1e", "#d4d4d4", "#2a2a2a", "#3c3c3c", "#78aeed", "#e0e0e0"),
        _ => ("#fafafa", "#2e2e2e", "#f0f0f0", "#d5d5d5", "#1c71d8", "#1e1e1e"),
    };
