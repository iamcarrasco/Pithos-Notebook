...
pub fn wire_editor_signals(ctx: &EditorCtx, tag_entry: &gtk::Entry) {
    ...
    // Focus mode + typewriter scrolling: update on cursor movement
    {
        let ctx = ctx.clone();
        let buffer = ctx.source_buffer.clone();
        buffer.connect_cursor_position_notify(move |_| {
            update_focus_mode(&ctx);
            typewriter_scroll(&ctx);
        });
    }
    ...
    let id = glib::timeout_add_local_once(
        ...
        move || {
            ...
            if !ctx_inner.state.borrow().suppress_sync {
                ...
                update_inline_link_tags(&ctx_inner);
            }
        },
    );
...
pub fn handle_source_view_keys(
    ...
    // Hemingway mode: block backspace/delete (forward-only writing)
    if ctx.state.borrow().hemingway_mode
...
// ---------------------------------------------------------------------------
// Focus mode: dim all text except the current paragraph
// ---------------------------------------------------------------------------

const FOCUS_DIM_TAG: &str = "focus-dim";

fn ensure_focus_dim_tag(buffer: &sourceview::Buffer) {
    let table = buffer.tag_table();
    if table.lookup(FOCUS_DIM_TAG).is_none() {
        let tag = gtk::TextTag::builder()
            .name(FOCUS_DIM_TAG)
            .foreground_rgba(&gtk::gdk::RGBA::new(0.5, 0.5, 0.5, 0.3))
            .build();
        table.add(&tag);
    }
}

pub fn update_focus_mode(ctx: &EditorCtx) {
    let buffer = &ctx.source_buffer;
    ensure_focus_dim_tag(buffer);

    let focus = ctx.state.borrow().focus_mode;
    if !focus {
        // Remove all dim tags when focus mode is off
        buffer.remove_tag_by_name(FOCUS_DIM_TAG, &buffer.start_iter(), &buffer.end_iter());
        return;
    }
    ...
    // Scan up for paragraph start
    let mut para_start_line = line;
    while para_start_line > 0 {
        let it = buffer.iter_at_line(para_start_line - 1).unwrap_or(buffer.start_iter());
        let mut end_it = it;
        if !end_it.ends_line() { end_it.forward_to_line_end(); }
        let text = buffer.text(&it, &end_it, true);
        if text.trim().is_empty() { break; }
        para_start_line -= 1;
    }

    // Scan down for paragraph end
    let mut para_end_line = line;
    while para_end_line < total_lines - 1 {
        let it = buffer.iter_at_line(para_end_line + 1).unwrap_or(buffer.end_iter());
        let mut end_it = it;
        if !end_it.ends_line() { end_it.forward_to_line_end(); }
        let text = buffer.text(&it, &end_it, true);
        if text.trim().is_empty() { break; }
    ...
    let para_start = buffer.iter_at_line(para_start_line).unwrap_or(buffer.start_iter());
    let mut para_end = buffer.iter_at_line(para_end_line).unwrap_or(buffer.end_iter());
    if !para_end.ends_line() { para_end.forward_to_line_end(); }

    // Apply dim to everything, then remove from current paragraph
    buffer.apply_tag_by_name(FOCUS_DIM_TAG, &buffer.start_iter(), &buffer.end_iter());
    buffer.remove_tag_by_name(FOCUS_DIM_TAG, &para_start, &para_end);
}
...
pub fn typewriter_scroll(ctx: &EditorCtx) {
    if !ctx.state.borrow().typewriter_scrolling { return; }
    let buffer = &ctx.source_buffer;
    let mark = buffer.get_insert();
    ctx.source_view.scroll_to_mark(&mark, 0.0, true, 0.5, 0.5);
}

// ---------------------------------------------------------------------------
// Inline link/image peek: apply tooltip tags for markdown links
// ---------------------------------------------------------------------------

const LINK_TOOLTIP_TAG: &str = "link-tooltip";

fn ensure_link_tooltip_tag(buffer: &sourceview::Buffer) {
    let table = buffer.tag_table();
    if table.lookup(LINK_TOOLTIP_TAG).is_none() {
        let tag = gtk::TextTag::builder()
            .name(LINK_TOOLTIP_TAG)
...
/// Scan the buffer for markdown links and images and apply tooltip tags.
/// Called from process_buffer_change.
pub fn update_inline_link_tags(ctx: &EditorCtx) {
    let buffer = &ctx.source_buffer;
    ensure_link_tooltip_tag(buffer);

    // Remove all existing link tooltip tags
    buffer.remove_tag_by_name(LINK_TOOLTIP_TAG, &buffer.start_iter(), &buffer.end_iter());

    let text = buffer.text(&buffer.start_iter(), &buffer.end_iter(), true).to_string();

    // Simple scanner for [text](url) and ![alt](url) patterns
    let bytes = text.as_bytes();
    let len = bytes.len();
    let mut i = 0;
    while i < len {
        // Check for ![ or [
        let is_image = i < len && bytes[i] == b'!';
        let bracket_start = if is_image { i + 1 } else { i };
        if bracket_start >= len || bytes[bracket_start] != b'[' {
            i += 1;
            continue;
        }
        ...
        // Apply tag to the entire [text](url) or ![alt](url) span
        let char_start = text[..i].chars().count() as i32;
        let char_end = text[..k].chars().count() as i32;
        let start_iter = buffer.iter_at_offset(char_start);
        let end_iter = buffer.iter_at_offset(char_end);
        buffer.apply_tag_by_name(LINK_TOOLTIP_TAG, &start_iter, &end_iter);

        i = k;
    }
}

/// Wire the query-tooltip signal for link/image peek on the source view.
pub fn wire_link_tooltips(ctx: &EditorCtx) {
    ctx.source_view.set_has_tooltip(true);
    let ctx = ctx.clone();
    ctx.source_view.connect_query_tooltip(move |view, x, y, _keyboard, tooltip| {
        ...
        let Some(link_tag) = tag_table.lookup(LINK_TOOLTIP_TAG) else { return false };
        if !iter.has_tag(&link_tag) { return false; }

        // Extract the URL: find the tag range, then parse the (url) part
        let mut start = iter;
        let mut end = iter;
        if !start.starts_tag(Some(&link_tag)) { start.backward_to_tag_toggle(Some(&link_tag)); }
        if !end.ends_tag(Some(&link_tag)) { end.forward_to_tag_toggle(Some(&link_tag)); }

        let full_text = buffer.text(&start, &end, true).to_string();
        // Extract URL from [text](url) or ![alt](url)
        if let Some(paren_start) = full_text.rfind('(') {
            let url = full_text[paren_start + 1..].trim_end_matches(')');
            if full_text.starts_with('!') {
                tooltip.set_text(Some(&format!("Image: {url}")));
            } else {
...
pub fn store_image_as_asset(ctx: &EditorCtx, data: &[u8], filename: &str, mime: &str) {
    ...
    std::thread::spawn(move || {
        let write_data = if let Some(ref key) = cached_key {
            match crate::crypto::encrypt_asset(&data_owned, key) {
        ...
        if let Err(e) = crate::vault::write_asset(&vault_folder, &asset_id_thread, &write_data) {
        ...
        match rx.try_recv() {
            Ok(Ok(())) => {
                let now = std::time::SystemTime::now()
                    ...
                    .unwrap_or_default()
                    .as_millis() as i64;
                let meta = crate::vault::AssetMeta {
                    id: asset_id.clone(),
...
pub fn check_link_autocomplete(ctx: &EditorCtx) {
    let buffer = &ctx.source_buffer;
    let cursor = buffer.iter_at_offset(buffer.cursor_position());
    let line_start = buffer.iter_at_line(cursor.line()).unwrap_or(cursor);
    let text = buffer.text(&line_start, &cursor, true).to_string();

    // Find last unclosed "[["
    if let Some(pos) = text.rfind("[[") {
        let after = &text[pos + 2..];
        // If there's already a "]]" after the "[[", it's closed
        if after.contains("]]") {
            ctx.link_popover.popdown();
            return;
        }
        let filter = after.to_lowercase();
        ...
        // Populate list
        while let Some(child) = ctx.link_list_box.first_child() {
            ctx.link_list_box.remove(&child);
        }
        for name in &names {
            ...
            ctx.link_list_box.append(&row);
        }

        // Position popover at cursor
        let location = ctx.source_view.iter_location(&cursor);
        let (x, y) = ctx.source_view.buffer_to_window_coords(
            gtk::TextWindowType::Widget,
            location.x(),
            location.y(),
        );
        ctx.link_popover.set_pointing_to(Some(&gdk::Rectangle::new(x, y + location.height(), 1, 1)));
        ctx.link_popover.popup();
    } else {
        ctx.link_popover.popdown();
    }
}

pub fn complete_wiki_link(ctx: &EditorCtx, note_name: &str) {
    let buffer = &ctx.source_buffer;
    let cursor_pos = buffer.cursor_position();
    let cursor = buffer.iter_at_offset(cursor_pos);
    let line_start = buffer.iter_at_line(cursor.line()).unwrap_or(cursor);
    let text = buffer.text(&line_start, &cursor, true).to_string();

    if let Some(pos) = text.rfind("[[") {
        let line_offset = line_start.offset();
        let bracket_offset = line_offset + pos as i32;
        let mut del_start = buffer.iter_at_offset(bracket_offset);
        let mut del_end = buffer.iter_at_offset(cursor_pos);
        buffer.delete(&mut del_start, &mut del_end);
        let replacement = format!("[[{note_name}]]");
        let mut ins = buffer.iter_at_offset(bracket_offset);
        buffer.insert(&mut ins, &replacement);
        process_buffer_change(ctx);
    }
    ctx.link_popover.popdown();
}

pub fn wire_link_autocomplete(ctx: &EditorCtx) {
    // Wire list box row activation
    {
        let link_list_box = ctx.link_list_box.clone();
        let ctx = ctx.clone();
        link_list_box.connect_row_activated(move |_, row| {
            if let Some(label) = row.child().and_then(|c| c.downcast::<gtk::Label>().ok()) {
                let name = label.text().to_string();
                complete_wiki_link(&ctx, &name);
            }
        });
    }
}
