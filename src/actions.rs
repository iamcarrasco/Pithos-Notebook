...
pub fn build_content_menu() -> gtk::gio::Menu {
    ...
    section1.append(Some("Copy as HTML"), Some("win.copy-html"));
    ...
    section2.append(Some("Focus Mode"), Some("win.toggle-focus-mode"));
    section2.append(Some("Hemingway Mode"), Some("win.toggle-hemingway"));
    section2.append(Some("Typewriter Scrolling"), Some("win.toggle-typewriter"));
    section2.append(Some("Cycle Preview Theme"), Some("win.cycle-preview-theme"));
...
pub fn wire_menu_actions(ctx: &EditorCtx) {
    ...
    // Copy as HTML
    let action = SimpleAction::new("copy-html", None);
    {
        let ctx = ctx.clone();
        action.connect_activate(move |_, _| {
            let md = source_buffer_text(&ctx.source_buffer);
            let html = crate::preview::markdown_to_html(&md);
            ctx.source_view.clipboard().set_text(&html);
            send_toast(&ctx, "HTML copied to clipboard");
        });
    }
    window.add_action(&action);

    // Hemingway mode (no backspace/delete)
    let action = SimpleAction::new("toggle-hemingway", None);
    {
        let ctx = ctx.clone();
        action.connect_activate(move |_, _| {
            let on = {
                let mut s = ctx.state.borrow_mut();
                s.hemingway_mode = !s.hemingway_mode;
                s.hemingway_mode
            };
            send_toast(&ctx, if on { "Hemingway mode on" } else { "Hemingway mode off" });
        });
    }
    window.add_action(&action);

    // Focus mode
    let action = SimpleAction::new("toggle-focus-mode", None);
    {
        let ctx = ctx.clone();
        action.connect_activate(move |_, _| {
            let on = {
            ...
            send_toast(&ctx, if on { "Focus mode on" } else { "Focus mode off" });
        });
    }
    window.add_action(&action);

    // Typewriter scrolling
    let action = SimpleAction::new("toggle-typewriter", None);
    {
        let ctx = ctx.clone();
        action.connect_activate(move |_, _| {
            let on = {
                let mut s = ctx.state.borrow_mut();
                s.typewriter_scrolling = !s.typewriter_scrolling;
                s.typewriter_scrolling
            };
            send_toast(&ctx, if on { "Typewriter scrolling on" } else { "Typewriter scrolling off" });
        });
    }
    window.add_action(&action);

    // Preview theme (cycles: default → sepia → solarized → default)
    let action = SimpleAction::new("cycle-preview-theme", None);
    {
        let ctx = ctx.clone();
        action.connect_activate(move |_, _| {
            let name = {
                let mut s = ctx.state.borrow_mut();
                s.preview_theme = match s.preview_theme.as_str() {
                    "default" => "sepia".to_string(),
                    "sepia" => "solarized".to_string(),
                    _ => "default".to_string(),
                };
                s.preview_theme.clone()
            };
            render_preview(&ctx);
            send_toast(&ctx, &format!("Preview theme: {name}"));
        });
    }
    window.add_action(&action);
