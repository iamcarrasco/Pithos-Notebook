...
pub fn refresh_note_list(ctx: &EditorCtx) {
    ...
    let (mut visible, active_id, sort_order, search_query, highlight_term, folders, filter_active) = {
        ...
        let expr = if has_search { Some(parse_search_query(&raw_query)) } else { None };
        let highlight_term = expr.as_ref().and_then(first_search_term).unwrap_or_default();

        let visible: Vec<NoteSummary> = state
            .notes
            .iter()
            .filter(|note| {
                let matches_search = match &expr {
                    None => true,
                    Some(e) => matches_search_expr(&note.name, &note.content, e),
                };
...
// ---------------------------------------------------------------------------
// Search query parser (supports "exact phrase", implicit AND, OR)
// ---------------------------------------------------------------------------

#[derive(Debug, PartialEq)]
enum SearchExpr {
    Term(String),       // single word (lowercase)
    Phrase(String),     // "exact phrase" (lowercase)
    And(Vec<SearchExpr>),
    Or(Box<SearchExpr>, Box<SearchExpr>),
}

/// Parse a search query: quoted strings → phrase, `OR` → disjunction, else AND.
fn parse_search_query(query: &str) -> SearchExpr {
    let tokens = tokenize_query(query);
    if tokens.is_empty() {
        return SearchExpr::And(vec![]);
    }

    // Group tokens around OR: split into OR-separated groups, each group is AND'd
    let mut groups: Vec<Vec<SearchExpr>> = vec![vec![]];
    for tok in tokens {
        if tok == "OR" {
            groups.push(vec![]);
        } else if tok.starts_with('"') && tok.ends_with('"') && tok.len() > 1 {
            let inner = tok[1..tok.len()-1].to_lowercase();
            if !inner.is_empty() {
                groups.last_mut().unwrap().push(SearchExpr::Phrase(inner));
            }
        } else {
            groups.last_mut().unwrap().push(SearchExpr::Term(tok.to_lowercase()));
        }
    }
    ...
    // Build: each group becomes an And, groups are chained with Or
    let mut exprs: Vec<SearchExpr> = groups
        .into_iter()
        .map(|g| if g.len() == 1 { g.into_iter().next().unwrap() } else { SearchExpr::And(g) })
        .collect();

    if exprs.len() == 1 {
        exprs.remove(0)
    } else {
        // Chain: a OR b OR c → Or(a, Or(b, c))
        let mut result = exprs.pop().unwrap();
        while let Some(left) = exprs.pop() {
            result = SearchExpr::Or(Box::new(left), Box::new(result));
        }
        result
    }
}

/// Tokenize query: split on whitespace but keep quoted strings together.
fn tokenize_query(query: &str) -> Vec<String> {
    ...
    while let Some(&ch) = chars.peek() {
        ...
        if ch == '"' {
            // Consume quoted phrase
            let mut tok = String::new();
            tok.push(chars.next().unwrap()); // opening "
            loop {
                match chars.next() {
                    Some('"') => { tok.push('"'); break; }
                    Some(c) => tok.push(c),
                    None => { tok.push('"'); break; } // unclosed quote
                }
            }
            tokens.push(tok);
        } else {
...
/// Test whether text matches the search expression (case-insensitive).
fn matches_search_expr(name: &str, content: &str, expr: &SearchExpr) -> bool {
    let name_lower = name.to_lowercase();
    let content_lower = content.to_lowercase();
    eval_expr(&name_lower, &content_lower, expr)
}

fn eval_expr(name: &str, content: &str, expr: &SearchExpr) -> bool {
    match expr {
        SearchExpr::Term(word) => name.contains(word.as_str()) || content.contains(word.as_str()),
        SearchExpr::Phrase(phrase) => name.contains(phrase.as_str()) || content.contains(phrase.as_str()),
        SearchExpr::And(children) => children.iter().all(|c| eval_expr(name, content, c)),
        SearchExpr::Or(a, b) => eval_expr(name, content, a) || eval_expr(name, content, b),
    }
}

/// Extract the first concrete search term for snippet/highlight generation.
fn first_search_term(expr: &SearchExpr) -> Option<String> {
    match expr {
        SearchExpr::Term(w) => Some(w.clone()),
        SearchExpr::Phrase(p) => Some(p.clone()),
        SearchExpr::And(children) => children.iter().find_map(first_search_term),
        SearchExpr::Or(a, _) => first_search_term(a),
    }
}
...
mod tests {
    ...
    fn test_single_word_search() {
        let expr = parse_search_query("rust");
        assert!(matches_search_expr("Rust notes", "", &expr));
        assert!(matches_search_expr("", "learning rust", &expr));
        assert!(!matches_search_expr("python notes", "no match", &expr));
    }

    #[test]
    fn test_implicit_and() {
        let expr = parse_search_query("rust memory");
        assert!(matches_search_expr("rust", "memory management", &expr));
        assert!(!matches_search_expr("rust", "no second term", &expr));
    }

    #[test]
    fn test_quoted_phrase() {
        let expr = parse_search_query("\"memory safety\"");
        assert!(matches_search_expr("", "rust memory safety is great", &expr));
        assert!(!matches_search_expr("", "memory and safety", &expr));
    }

    #[test]
    fn test_or_operator() {
        let expr = parse_search_query("rust OR python");
        assert!(matches_search_expr("rust", "", &expr));
        assert!(matches_search_expr("python", "", &expr));
        assert!(!matches_search_expr("java", "", &expr));
    }

    #[test]
    fn test_complex_query() {
        // rust "memory safety" OR gc
        let expr = parse_search_query("rust \"memory safety\" OR gc");
        // Must match: (rust AND "memory safety") OR gc
        assert!(matches_search_expr("rust", "memory safety rocks", &expr));
        assert!(matches_search_expr("gc", "", &expr));
        assert!(!matches_search_expr("rust", "just rust", &expr));
    }

    #[test]
    fn test_empty_query() {
        let expr = parse_search_query("");
        assert!(matches_search_expr("anything", "anything", &expr));
    }

    #[test]
    fn test_first_search_term() {
        let expr = parse_search_query("rust \"memory safety\"");
        assert_eq!(first_search_term(&expr), Some("rust".into()));
    }
}
