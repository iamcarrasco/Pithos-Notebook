...
pub fn toggle_zen_mode(ctx: &EditorCtx) {
    ...
    if zen {
        // Save current split position before entering zen mode (offset by 1 to
        // distinguish "never saved" from a genuine position of 0).
        ctx.pre_zen_split_pos.set(ctx.split.position() + 1);
        ...
        let saved = ctx.pre_zen_split_pos.get();
        ctx.split.set_position(if saved > 0 { saved - 1 } else { ctx.split.allocated_width() / 2 });
