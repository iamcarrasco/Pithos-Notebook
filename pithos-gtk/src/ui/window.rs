...
/// Sidebar collapses to overlay below this width (toolbar ~960px + sidebar ~300px).
const SIDEBAR_COLLAPSE_BREAKPOINT: &str = "max-width: 1260sp";
...
pub fn build_editor(
    ...
    match adw::BreakpointCondition::parse(SIDEBAR_COLLAPSE_BREAKPOINT) {
        Ok(cond) => {
    ...
    // Show vault folder name in the header
    {
        let vf = ctx.vault_folder.borrow();
        let vault_display = std::path::Path::new(vf.as_str())
            .file_name()
            .and_then(|n| n.to_str())
            .unwrap_or("Vault");
        ctx.vault_name_label.set_label(vault_display);
    }
