...
pub fn wire_keyboard_shortcuts(ctx: &EditorCtx, window: &adw::ApplicationWindow) {
    ...
    // Source-view key handler: Escape (zen exit), Ctrl+V (paste image), Enter (list continuation).
    // CAPTURE phase is required so we intercept Enter before GtkSourceView inserts the newline.
    {
        let source_view = ctx.source_view.clone();
        let ctx = ctx.clone();
...
pub fn process_buffer_change(ctx: &EditorCtx) {
    ...
    // Keep cursor visible after programmatic buffer changes (prevents scroll jump)
    let mut cursor = ctx
        .source_buffer
        .iter_at_offset(ctx.source_buffer.cursor_position());
    ctx.source_view
        .scroll_to_iter(&mut cursor, 0.0, false, 0.0, 0.0);
}
...
pub fn md_wrap(buffer: &sourceview::Buffer, prefix: &str, suffix: &str, placeholder: &str) {
    ...
    if let Some((start, end)) = buffer.selection_bounds() {
        ...
        let suffix_len = suffix.chars().count() as i32;

        if is_wrap_applied(buffer, &start, &end, prefix, suffix) {
            // Toggle OFF: single delete of prefix+selected+suffix, then re-insert selected
            let unwrap_start = anchor - prefix_len;
            let mut ds = buffer.iter_at_offset(unwrap_start);
            let mut de = buffer.iter_at_offset(end_off + suffix_len);
            buffer.delete(&mut ds, &mut de);
            let mut ins = buffer.iter_at_offset(unwrap_start);
            buffer.insert(&mut ins, &selected);
            // Re-select
            let sel_start = buffer.iter_at_offset(unwrap_start);
            let sel_end = buffer.iter_at_offset(
                unwrap_start + selected.chars().count() as i32,
            );
            buffer.select_range(&sel_start, &sel_end);
        } else {
            // Toggle ON: wrap with prefix/suffix
            ...
            let mut ins = buffer.iter_at_offset(anchor);
            buffer.insert(&mut ins, &text);
            // Re-select the wrapped content (excluding markers)
            let sel_start = buffer.iter_at_offset(anchor + prefix_len);
            let sel_end = buffer.iter_at_offset(
                anchor + prefix_len + selected.chars().count() as i32,
            );
            buffer.select_range(&sel_start, &sel_end);
        }
    } else {
...
/// Check whether the format markers already surround the selection.
/// Handles the bold (`**`) / italic (`*`) overlap correctly by counting
/// consecutive `*` characters rather than doing a simple string match.
fn is_wrap_applied(
    buffer: &sourceview::Buffer,
    sel_start: &gtk::TextIter,
    sel_end: &gtk::TextIter,
    prefix: &str,
    suffix: &str,
) -> bool {
    let prefix_len = prefix.chars().count() as i32;
    let anchor = sel_start.offset();
    if anchor < prefix_len {
        return false;
    }

    // Special logic for asterisk-based markers (bold/italic interaction)
    if prefix == suffix && prefix.chars().all(|c| c == '*') {
        let stars_before = count_consecutive(buffer, sel_start, '*', true);
        let stars_after = count_consecutive(buffer, sel_end, '*', false);
        let min_stars = stars_before.min(stars_after);
        return if prefix_len == 2 {
            min_stars >= 2 // bold: at least 2 stars on each side
        } else {
            min_stars % 2 == 1 // italic: odd number of stars (1 or 3)
        };
    }

    // General case: exact prefix/suffix match around selection
    let before = buffer.iter_at_offset(anchor - prefix_len);
    let before_text = buffer.text(&before, sel_start, true);
    let suffix_len = suffix.chars().count() as i32;
    let after = buffer.iter_at_offset(sel_end.offset() + suffix_len);
    let after_text = buffer.text(sel_end, &after, true);
    before_text == prefix && after_text == suffix
}

/// Count consecutive occurrences of `ch` immediately before (`backward=true`)
/// or after (`backward=false`) the given iterator position.
fn count_consecutive(
    ...
    let mut count = 0;
    let mut pos = iter.offset();
    if backward {
        while pos > 0 {
            let it = buffer.iter_at_offset(pos - 1);
            if it.char() == ch {
                count += 1;
                pos -= 1;
            } else {
        ...
        let end = buffer.end_iter().offset();
        while pos < end {
            let it = buffer.iter_at_offset(pos);
            if it.char() == ch {
                count += 1;
                pos += 1;
            } else {
