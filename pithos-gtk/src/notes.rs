...
pub fn refresh_tags(ctx: &EditorCtx) {
    ...
    ctx.tag_entry
        .set_placeholder_text(Some("Add tag and press Enter\u{2026}"));
...
pub fn builtin_templates() -> Vec<(String, String, String)> {
    pithos_core::templates::builtin_templates()
}
...
pub fn show_template_picker(ctx: &EditorCtx) {
    ...
    if !custom_templates.is_empty() {
        ...
        for (name, body, tags_csv) in custom_templates.iter() {
            if disabled.contains(name) {
                continue;
            }
                ...
                del_btn.connect_clicked(move |_| {
                    {
                        ...
                        if let Some(pos) = state
                            .custom_templates
                            .iter()
                            .position(|(n, _, _)| n == &tpl_name)
                        {
                            state.custom_templates.remove(pos);
                        }
                    }
                    trigger_vault_save(&ctx);
                    dialog.close();
